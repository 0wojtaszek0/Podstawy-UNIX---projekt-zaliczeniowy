...
> najprościej zapisać je jako zwykłe łańcuchy: `DEST_DIR="/mnt/backups"`.
...
4. **Opcje `rsync`** budowane jako tablica `rsync_opts`:
   ...
   - `--delete` — usuwa z kopii pliki, których **już nie ma** w źródle
     (kopia jest lustrem stanu bieżącego, nie „narastającym śmietnikiem”).
...
| `1` | Błąd krytyczny (`die`): zła konfiguracja, brak dostępu, błąd `rsync`, druga instancja działa, przerwanie sygnałem. |
...
- **Szablon `.conf`** — `DEST_DIR`/`LOG_FILE` w repo są zapisane jako puste
  tablice; przy użyciu wypełnij je jako łańcuchy (patrz sekcja 4).
